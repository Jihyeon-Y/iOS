...
class AddNewViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UITextFieldDelegate {
    ...
    @IBAction func saveButton(_ sender: UIButton) {
        ...
        if let name = nameField.text, !name.isEmpty, let breed = breedField.text, !breed.isEmpty, let weight = weightField.text, !weight.isEmpty {
            let newPet = Pet(image: imageView.image, name: name, birthday: datePicker.date, breed: breed, gender: genderSegue.selectedSegmentIndex, weight: weight)
            petList.pets.append(newPet)
